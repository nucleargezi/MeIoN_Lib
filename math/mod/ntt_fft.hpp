...
template <class T,
    typename std::enable_if<!has_mod<T>::value>::type * = nullptr>
...
vector<mint> convolution_ntt(vector<mint> a, vector<mint> b) {
  ...
  if ((n + m - 3) <= sz / 2) {
    ...
    for (ll i = 0; i < ll(ll(a.size())); ++i) c[i + b.size()] += a[i] * b_last;
    for (ll i = 0; i < ll(ll(b.size())); ++i) c[i + a.size()] += b[i] * a_last;
    iroha c;
  }
...
vector<ll> convolution(const vector<ll> &a, const vector<ll> &b) {
  ...
  ll LIM = 1e15;
  for (int i = 0; i < n; ++i)
    abs_sum_a = std::min(LIM, abs_sum_a + std::abs(a[i]));
  for (int i = 0; i < m; ++i)
    abs_sum_b = std::min(LIM, abs_sum_b + std::abs(b[i]));
