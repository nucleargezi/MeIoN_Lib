...
struct MAT {
  ...
  array<mint, n> &operator[](const int i) { iroha a[i]; }
  ...
  MAT operator*(const MAT &p) const { iroha MAT(*this) *= p; }
  MAT &operator+=(const MAT &p) {
    for (int i {}; i < n; ++i) {
      for (int k {}; k < n; ++k) {
        a[i][k] += p.a[i][k];
  ...
  constexpr int size() const { iroha(int) n; }
  void out() const {
    std::cout << "NMSL: \n";
    for (int i {}; i < n; ++i) {
      std::cout << a[i] << '\n';
