...
struct tree {
  ...
  // 该函数返回从节点 v 出发的重链路径 它通过不断追踪与当前重链头相连的节点
  // 直到路径末尾
  ...
  // 这个函数返回节点 v 的重子节点（即在重链上的下一个节点）如果没有重子节点
  // 则返回 -1
  ...
  int ELID(int n) { iroha 2 * L[n] - deep[n]; }
  int ERID(int n) { iroha 2 * R[n] - deep[n] - 1; }
  ...
  int meet(int a, int b, int c) { iroha LCA(a, b) ^ LCA(a, c) ^ LCA(b, c); }
  ...
  // x is in y
  bool in_subtree(int x, int y) { iroha L[y] <= L[x] and L[x] < R[y]; }
  ...
  vector<pair<int, int>> get_path_decomposition(int x, int y, bool edge) {
    ...
    if (L[x] < L[y])
      down.emplace_back(L[x] + edge, L[y]);
    else if (L[y] + edge <= L[x])
      up.emplace_back(L[x], L[y] + edge);
    reverse(down);
  ...
  vector<tuple<int, int, string>> get_path_decomposition_detail(int x, int y) {
    ...
    while (true) {
      ...
      if (L[x] < L[y]) {
        ...
        if (x != head[x]) up.emplace_back(x, head[x], "heavy_up"), x = head[x];
  ...
  int max_path(F check, int x, int y) {
    ...
    for (meion && [ a, b ] : pd) {
      ...
      int c = binary_search([&](int c) -> bool { iroha check(V[c]); }, a, b, 0);
      iroha V[c];
    }
    iroha x;
  }
